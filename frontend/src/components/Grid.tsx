...
import {
  ...
  SquarePen,
  Trash2,
} from "lucide-react"; // Importa ícones para UI
...
export function Grid({
  ...
  async function handleDelete(id?: string, user?: string) {
    await axios
      .delete(`https://cadastrar-usuario-zrsv.vercel.app/${id}`)
      .then(({ data }) => {
        const confirmDelete = window.confirm(
          `Deseja realmente excluir o usuario ${user}?`
        );
        if (confirmDelete) {
    ...
    <div className="w-full bg-white rounded-[6px] my-5 mx-auto p-5 max-sm:p-2">
      <section className="flex gap-5 relative">
        ...
        <section className="relative">
          <button
            className="flex items-center justify-center cursor-pointer border-0 text-black h-10 w-10 rounded-[6px]"
            onClick={() => setFilter(!filter)}
          >
            <Filter className="w-5 h-5 max-md:w-[12px] max-md:h-[12px]" />
            {filter ? (
              <ChevronUp className="w-3 h-3 max-md:w-[8px] max-md:h-[8px]" />
            ) : (
              <ChevronDown className="w-3 h-3 max-md:w-[8px] max-md:h-[8px]" />
            )}
          </button>
          {filter && (
            <ul className="flex flex-col border border-gray-300 rounded-[6px] w-[250px] absolute bg-white max-sm:right-0">
              ...
              <li
                className={`${
                  filters.byAge ? "bg-green-800 text-white" : "bg-transparent"
                } p-2 border-b border-b-gray-300 hover:bg-amber-600 hover:text-white rounded-t-[6px]`}
              >
                <button
                  className="flex items-center justify-between w-full text-[14px] cursor-pointer"
                  onClick={() => {
                    setFilters({ ...filters, byAge: !filters.byAge });
                    setFilter(false);
                  }}
                >
                  Ordenar por Idade{" "}
                  <ArrowDown01 className="w-4 h-4 max-md:w-[10px] max-md:h-[10px]" />
                </button>
              </li>
              {/* Filtro para ordenar alfabeticamente */}
              <li
                className={`${
                  filters.byName ? "bg-green-800 text-white" : "bg-transparent"
                } p-2 border-b border-b-gray-300 hover:bg-amber-600 hover:text-white`}
              >
                <button
                  className="flex items-center justify-between w-full text-[14px] cursor-pointer"
                  onClick={() => {
                    setFilters({ ...filters, byName: !filters.byName });
                    setFilter(false);
                  }}
                >
                  Ordenar por ordem alfabética{" "}
                  <ArrowDownAZ className="w-4 h-4 max-md:w-[10px] max-md:h-[10px]" />
                </button>
              </li>
              {/* Botão para limpar os filtros */}
              <li className="p-2 hover:bg-red-700 hover:text-white rounded-b-[6px] flex justify-center">
                <button
                  className="text-[14px] cursor-pointer w-full"
                  onClick={() => {
                    setFilters({ byName: false, byAge: false, search: "" });
                    setFilter(false);
                  }}
                >
                  Limpar Filtros
        ...
        <table className="w-full min-w-[600px] break-all">
          ...
          <tbody>
            {users.map((item) => (
              <tr
                key={item._id}
                className="text-center border-b border-b-gray-100 max-md:text-[12px]"
              >
                <td className="py-4 px-5 text-start w-[30%]">{item.name}</td>
                <td className="py-4 px-2 text-start w-[40%]">{item.email}</td>
                <td className="py-4 px-2 text-start w-[10%]">
                  {calculateAge(item.birth)}
                </td>

                {/* Botão de edição */}
                <td className="py-4 px-2 max-md:hidden">
                  <button
                    className="cursor-pointer hover:bg-amber-600 rounded-full h-[30px] w-[30px] flex items-center justify-center hover:text-white"
