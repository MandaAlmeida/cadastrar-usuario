...
export function Form({ onEdit, setOnEdit, fetchUsers, users }: Props) {
    ...
    if (onEdit._id) {
      try {
        const { data } = await axios.put(
          `https://cadastrar-usuario-zrsv.vercel.app/${onEdit._id}`,
          user
        );
      ...
      if (users.some((existingUser) => existingUser.email === formData.email)) {
        ...
        try {
          const { data } = await axios.post(
            "https://cadastrar-usuario-zrsv.vercel.app/auth/register",
            user
  ...
  return (
    <form
      className="flex items-end gap-2.5 flex-wrap bg-white p-5 rounded-[6px] max-md:p-4 max-sm:flex-col max-sm:items-center max-md:justify-center max-md:text-[14px]"
